...
\begin{parts}
    ...
    \begin{mdframed}[backgroundcolor=gray!10]\begin{answer}
        ...
        \begin{tabular}{ p{4cm} | p{4cm} | p{3cm} | p{2cm} }
            ...
            [ROOT] & [I, attended, lectures, in, the, NLP, class] &  & Initial Configuration \\ \hline
            [ROOT, I] & [attended, lectures, in, the, NLP, class] &  &  \texttt{SHIFT}  \\ \hline
            [ROOT, I, attended] & [lectures, in, the, NLP, class] &  &  \texttt{SHIFT}  \\ \hline
            [ROOT, attended] & [lectures, in, the, NLP, class] & attended$\to$I &  \texttt{LEFT-ARC}  \\ \hline
            [ROOT, attended, lectures] & [in, the, NLP, class] &  & \texttt{SHIFT} \\ \hline
            [ROOT, attended] & [in, the, NLP, class] & attended$\to$lectures & \texttt{RIGHT-ARC} \\ \hline
            [ROOT, attended, in] & [the, NLP, class] &  & \texttt{SHIFT} \\ \hline
            [ROOT, attended, in, the] & [NLP, class] &  & \texttt{SHIFT} \\ \hline
            [ROOT, attended, in, the, NLP] & [class] &  & \texttt{SHIFT} \\ \hline
            [ROOT, attended, in, the, NLP, class] & [] &  & \texttt{SHIFT} \\ \hline
            [ROOT, attended, in, the, class] & [] & class$\to$NLP & \texttt{LEFT-ARC} \\ \hline
            [ROOT, attended, in, class] & [] & class$\to$the & \texttt{LEFT-ARC} \\ \hline
            [ROOT, attended, class] & [] & class$\to$in & \texttt{LEFT-ARC} \\ \hline
            [ROOT, attended] & [] & attended$\to$class & \texttt{RIGHT-ARC} \\ \hline
    ...
    \part[2] A sentence containing $n$ words will be parsed in how many steps (in terms of $n$)? Briefly explain in 1--2 sentences why.
       \begin{mdframed}[backgroundcolor=gray!10]\begin{answer}
            Cosidering the added ROOT, there are $n + 1$ words which will be shifted to the stack and be removed at a time. And every steps are one of the two conditions. Because ROOT will not be removed, there are:
   ...
   In \texttt{parser\_model.py} you will find skeleton code to implement this simple neural network using PyTorch. Complete the \texttt{\_\_init\_\_}, \texttt{embedding\_lookup} and \texttt{forward} functions to implement the model. Then complete the \texttt{train\_for\_epoch} and \texttt{train} functions within the \texttt{run.py} file.
    ...
    \begin{mdframed}[backgroundcolor=gray!10]\begin{answer}
        \textbf{Betst dev UAS:}88.56 and \textbf{Best text UAS:}89.32
    \end{answer} \end{mdframed}
...
\begin{mdframed}[backgroundcolor=gray!10]\begin{answer}
    POS tags will offer more information about the synactic and semantic meaning, which will improve the accuracy. \\
    What's more, the parser will be more efficient in parsing by using these tags. 
\end{answer} \end{mdframed}
