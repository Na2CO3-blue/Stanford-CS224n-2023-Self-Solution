...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        minibach = unfinished_parses[:batch_size]
        transitions = model.predict(minibach)
        for i, transition in enumerate(transitions):
            minibach[i].parse_step(transition)
            if (not minibach[i].buffer and len(minibach[i].stack) == 1):
                unfinished_parses.remove(minibach[i])
