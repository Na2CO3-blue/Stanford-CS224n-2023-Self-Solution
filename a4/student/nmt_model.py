...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = nn.Conv1d(self.embed_size, self.embed_size, kernel_size=2, padding='same')
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded) # type: ignore
        
        X = self.post_embed_cnn(torch.permute(X, (1, 2, 0)))
        X = torch.permute(X, (2, 0, 1))
        
        enc_hiddens, (last_hidden, last_cell) = self.encoder(pack_padded_sequence(X, source_lengths)) # type: ignore
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)
        enc_hiddens = torch.permute(enc_hiddens, (1, 0, 2)) # type: ignore
 
        init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), 1)) # type: ignore
        init_decoder_cell = self.c_projection(torch.cat((last_cell[0], last_cell[1]), 1)) # type: ignore
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(enc_hiddens) # type: ignore
        
        Y = self.model_embeddings.target(target_padded) # type: ignore
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        O_t = self.dropout(torch.tanh(V_t)) # type: ignore
