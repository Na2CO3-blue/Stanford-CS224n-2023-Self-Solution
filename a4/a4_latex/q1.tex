...
\end{lstlisting}
    ...
    \ifans{In the step function, $\be_f$ will be $-\infty$ if \texttt{en\_masks} is labeled 1. Then after softmax function, the attention value $\alpha_t$ will be $0$, which makes the padded elements are ignored in attention calculation. \\ Because the `pad' tokends are used for maintaining the same length of input sentences, they are useless in meaning representation. Thus, they should not be attentioned by encoder.}
\end{parts}
...
\end{lstlisting}
    ...
    \ifans{
        \begin{enumerate}[i.]
            \item \textit{dot product attention} just take the simple product of hidden state vector and query vector. This is the simpliest way to calculate the basic similarity. Compared to \textit{multiplicative attention}, this way didn't have the weight matrix. So it can be calculate fast and the model will be convergence faster. \\ However, the simple way take the entire input vectors as the same importance, which is a kind of wrong because the truth are we just use part of the information to calculate score and different parts count vary.
            \item \textit{additive attention} can assign different weight metrix to query vector and key vector, which enhance the ability to capture information and patterns form data. \\ The disadvantage of this way is that threre are more parameters to calculate. It is more expensive to calculate.
        \end{enumerate}}
\end{parts}
