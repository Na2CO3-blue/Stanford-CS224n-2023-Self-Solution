...
def sigmoid(x):  # sourcery skip: inline-immediately-returned-variable
    """
...
def negSamplingLossAndGradient(
    ...
    usedOutsideWordVecs = outsideVectors[indices]
    sigmoidUV = sigmoid(- usedOutsideWordVecs @ centerWordVec)
    sigmoidUV[0] = 1 - sigmoidUV[0]
    
    loss = -np.log(sigmoidUV[0]) - np.sum(np.log(sigmoidUV[1:]))
    
    sigmoidUV = 1 - sigmoidUV
    gradCenterVec = -(sigmoidUV[0]) * usedOutsideWordVecs[0] + sigmoidUV[1 : ] @ usedOutsideWordVecs[1 : ]
    
    sigmoidUV[0] *= -1
    gradOutsideVecs = np.zeros_like(outsideVectors)
    unique, sameIndices, counts = np.unique(indices, return_index = True, return_counts = True) # type: ignore
    gradOutsideVecs[unique, :] = centerWordVec * (sigmoidUV[sameIndices] * counts)[:, np.newaxis]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        partLoss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec, word2Ind[word], outsideVectors, dataset)
        loss += partLoss
