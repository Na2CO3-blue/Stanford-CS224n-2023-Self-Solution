...
\newcommand{\hmwkAuthorName}{Zinan Tang} % Your name
...
\begin{answer}
    ...
    \begin{align*}
        \frac{\partial \bm J_{\text{naive-softmax}}(\bm v_c, o, \bm U)}{\partial \bm u_o} & = - \bm v_c + \frac{\frac{\partial (\sum_{x \in \text{Vocab}} \exp(\bm u_{x}^\top \bm v_c))}{\partial \bm u_o}}{\sum_{w \in \text{Vocab}} \exp(\bm u_{w}^\top \bm v_c)} \\
        & = - \bm v_c + \frac{\bm v_c \exp(\bm u_{o}^\top \bm v_c)}{\sum_{w \in \text{Vocab}} \exp(\bm u_{w}^\top \bm v_c)} \\
        & = - \bm v_c + \bm v_c P(O = o | C = c) \\
        & = - \bm v_c + \bm y_o^\top \bm v_c
    \end{align*}

    When $w \neq o$, same as equations above, for any such $w$:

    \begin{align*}
        \frac{\partial \bm J_{\text{naive-softmax}}(\bm v_c, o, \bm U)}{\partial \bm u_w} 
        & = \frac{\frac{\partial (\sum_{x \in \text{Vocab}} \exp(\bm u_{x}^\top \bm v_c))}{\partial \bm u_w}}{\sum_{w \in \text{Vocab}} \exp(\bm u_{w}^\top \bm v_c)} \\
        & = \frac{\bm v_c \exp(\bm u_{w}^\top \bm v_c)}{\sum_{w \in \text{Vocab}} \exp(\bm u_{w}^\top \bm v_c)} \\
        & = \bm v_c P(O = w | C = c) \\
        & = \bm y_w^\top \bm v_c
    \end{align*}

    We can use matrix ro present two cases in one formula using $\hat{\bm y}, \bm y, \bm v_c$:

    \[\frac{\partial \bm J_{\text{naive-softmax}}(\bm v_c, o, \bm U)}{\partial \bm u_w} = (\hat{\bm y} - \bm y)^\top \bm v_c \tag{**} \label{partial J for u}\]

    because $\bm y$ is a one-hot vector that only when $w = o$, the element of it is 1.
\end{answer}
\end{shaded}
...
\[\frac{\partial J_{\text{neg-sample}}(\bm v_c, o, \bm U)}{\partial \bm u_{w_s}} = (1 - \sigma(-\bm u_{w_s}^\top \bm v_c))\bm v_c\]
\color{red}
Conider the equal and not equal condition, we can justify the equation as:
\[\frac{\partial J_{\text{neg-sample}}(\bm v_c, o, \bm U)}{\partial \bm u_{w_s}} = \sum_{\substack{1 \le x \le K\\ 1 \le s \le K \\ w_x = w_s}} (1 - \sigma(-\bm u_{w_x}^\top \bm v_c))\bm v_c\]
\end{answer}
\end{shaded}
